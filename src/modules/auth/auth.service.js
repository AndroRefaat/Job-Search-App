...
import { OTPS, providers, subjects } from '../../utils/enums/allEnums.js';
...
export const login = asyncHandler(async (req, res, next) => {
    ...
    if (!passwordMatch) return next(new Error("Invalid password", { cause: 400 }));

    if (user.isConfirmed === false) return next(new Error("please confirm your email", { cause: 400 }));
...
export const forgotPassword = asyncHandler(async (req, res, next) => {
    ...
    await OTP.create({ email, otps: [{ code: hashOTP, type: OTPS.forgetPassword }] })
...
export const resetPassword = asyncHandler(async (req, res, next) => {
    ...
    const isMatch = await compare({ plainText: otp, hash: otpRecord.otps.find(otp => otp.type === OTPS.forgetPassword).code });
    if (!isMatch) return next(new Error("Invalid OTP", { cause: 400 }));
