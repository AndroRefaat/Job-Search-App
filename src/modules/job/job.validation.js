...
export const getJobsMatchSchema = joi.object({
    page: joi.number(),
    limit: joi.number(),
    workingTime: joi.string(),
    seniorityLevel: joi.string(),
...
export const AcceptOrRejectApplicationSchema = joi.object({
    applicationId: joi.custom(isValidObjectId).required(),
    status: joi.string().valid(...Object.values(allStatus)).required()
}).required();