...
export const banOrUnbanCompany = asyncHandler(async (req, res, next) => {
    ...
    if (company.isBanned) {
        ...
        await company.save();
        return res.status(200).json({ success: true, message: "Company Unbanned successfully" })
    } else {
        company.bannedAt = Date.now();
        company.isBanned = true;
        await company.save();
        return res.status(200).json({ success: true, message: "Company Banned successfully" })
    }
...
export const approve = asyncHandler(async (req, res, next) => {
    ...
    if (company.approvedByAdmin) return next(new Error("company already approved", { cause: 400 }));
